...
def login(request):
    ...
    user = User.objects.filter(email=identifier).first() or \
           ...
           User.objects.filter(mobile=identifier).first()
    ...
    response.delete_cookie('admin_email')               # remove admin cookie if set
...
def admin_dashboard(request):
    ...
    # make sure field name used matches model (created_at/date)
...
def create_order(request, med_id):
    # allow only POST for order creation
    if request.method != 'POST':
        from django.contrib import messages
        messages.error(request, 'Invalid request method for ordering.')
        return redirect('medstore_app:home')

    # check user logged in via cookie
    user_email = request.COOKIES.get('user_email')
    if not user_email:
...
def admin_view_messages(request):
    ...
    messages_list = ContactMessage.objects.all().order_by('-created_at')[:50]
...
def admin_view_orders(request):
    """
    Simple admin orders list view.
    Orders ordered by a timestamp field if available, else by id.
    """
